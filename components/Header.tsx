        ...
        <header className={`flex items-center justify-between w-full z-50 px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-15 transition-all duration-300
            ...
            <div className="space-y-3 sm:space-y-5 hidden lg:block">
                <ul className={`justify-end text-sm font-medium ${scroll ? "hidden" : "flex flex-wrap gap-2"}`}>
                    <li className="flex items-center px-2 xl:px-4 border-r border-r-black text-nowrap hover:text-gray-600 cursor-pointer">
                        <BiSolidPhoneCall className="text-xl"/>+1 (555) 123-4567
                    </li>
                    <li className="flex items-center gap-x-1 px-2 xl:px-4 border-r border-r-black hover:text-gray-600 cursor-pointer">
                        <FaCircleQuestion/>Store Hours
                    </li>
                    <li className="px-2 xl:px-4 border-r border-r-black text-nowrap hover:text-gray-600 cursor-pointer">Loyalty Program</li>
                    <li className="px-2 xl:px-4 border-r border-r-black hover:text-gray-600 cursor-pointer">Delivery</li>
                    <li className="px-2 xl:px-4 border-r border-r-black hover:text-gray-600 cursor-pointer">Careers</li>
                    <li className="px-2">
                        <input 
                            type="search" 
                            placeholder="Search products..." 
                            className="pl-2 text-sm font-normal rounded-md text-black bg-white focus:outline-none focus:ring-2 focus:ring-blue-400 w-32 sm:w-40"
                            value={searchQuery}
                    ...
                    {menu.map((item) =>(
                        <li key={item.name}
                            className={`text-base xl:text-lg text-nowrap font-semibold flex items-center cursor-pointer hover:border-b-2
                                ${pathname=== item.href && scroll ? "border-b-2 border-blue-400" : pathname=== item.href && !scroll ? "border-b-2 border-blue-400" : ""}`
                            }
                        >
                            <a href={item.href}>{item.name}</a>
                        </li>
                    ))}
                    <li onClick={() => push("/login")} className="p-2 bg-blue-400 text-white text-base xl:text-lg text-nowrap font-semibold cursor-pointer rounded-b-2xl rounded-tl-2xl hover:bg-blue-500">
                        LOGIN/SIGNUP
                    </li>
                </ul>
            </div>
            ...
            {open && (
                <nav className="absolute top-0 right-0 h-screen w-64 sm:w-80 bg-gray-100 shadow-lg z-20 text-blue-400 lg:hidden">
                    <button className="p-4 text-2xl cursor-pointer hover:text-blue-500" onClick={() => setOpen(false)}>
                        <IoClose/>
                    </button>
